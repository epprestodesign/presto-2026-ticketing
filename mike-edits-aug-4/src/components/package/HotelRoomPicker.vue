...
<template>
<section v-if="hasStay" class="pqv__rooms">
  <h3 class="pqv__h3">Choose your hotel &amp; room</h3>
  <p class="pqv__sub">
    {{ hotelOptions.length }} hotels are covered by this package. Prices shown are the
    difference against what the package already includes.
  </p>

  <div v-for="h in hotelOptions" :key="h.id" class="hotel" :class="{ 'hotel--open': openHotelId === h.id }">
    <button type="button" class="hotel__head" @click="emit('toggle-hotel', h)">
...
.hotel__head { display: grid; grid-template-columns: 84px minmax(0, 1fr) auto auto; align-items: center; gap: 14px; width: 100%; padding: 12px 14px; background: none; border: 0; text-align: left; cursor: pointer; font: inherit; }
