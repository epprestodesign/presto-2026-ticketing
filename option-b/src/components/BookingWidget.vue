<script setup>
// BookingWidget — LOCAL FORK for the "Aug 4 — Option B" prototype only.
//
...
// one is a plain "Guests" dropdown of 1–8+. The fork is wired in by a resolveId
// override in this app's vite.config.js, so the three library pages that embed the
// widget (LandingPage, HotelListPage, HotelDetailPage) pick it up here — and ONLY
// here. Storybook and the other prototypes still get the library version.
//
...
import { EVENT_DATE_ISO } from '../event.js'
import { journey, setGuests } from '../store.js'
...
// --- Dates ---
// FORK: the library defaults this to today + 7 → + 10 days, which floats away
// from the event (on Aug 4 it read "Aug 11 – 14"). This prototype is about ONE
// game, so the search opens on the gameday weekend instead: check in the day
// before kickoff, check out the day after — Sat Sep 19 → Mon Sep 21, a 2-night /
// 3-day stay around Sun Sep 20's 1:00 PM kickoff.
const dstr = (offset) => {
...
const range = ref({ from: dstr(-1), to: dstr(1) })
