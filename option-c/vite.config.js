  ...
  transform(code, id) {
    ...
    for (const [from, to] of patch.edits) {
      if (!out.includes(from)) {
        throw new Error(
          `[option-c] ${patch.file} no longer contains:\n  ${from}\n` +
          `The library changed — update PATCHES in option-c/vite.config.js.`
        )
      }
...
// `/presto-2026-ticketing/option-c/`; local dev serves from `/`. The
// deploy workflow passes `--base=/presto-2026-ticketing/option-c/`.
