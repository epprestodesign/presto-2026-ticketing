...
Use the **\`count\`** control to pad the matching results and stress-test the
filters/results list at scale (default keeps the curated sample).
` } } },
  argTypes: {
    count: {
      name: 'Results (count)',
...
const page = (showTeams) => (args) => ({
  ...
  template: `<page-frame cart-mode="hold"><hotel-list-page flow="group" :show-teams="showTeams" :count="args.count" /></page-frame>`,
})
